...
class PhanSo:
    ...
    def __init__(self, tu: int, mau: int):
        ...
        self.tu  = tu       # dùng setter
        self.mau = mau      # dùng setter
    ...
    def is_toi_gian(self) -> bool:
        """Kiểm tra phân số có ở dạng tối giản chưa."""
    ...
    def _gia_tri(self) -> float:
        """Giá trị thực để so sánh và tính toán."""
    ...
    def __truediv__(self, other: "PhanSo") -> "PhanSo":
        if other._tu == 0:
            raise MauSoBangKhong("Không thể chia cho phân số bằng 0!")
    ...
    def __eq__(self, other) -> bool:
        ...
        # So sánh theo giá trị thực (2/4 == 1/2)
        a = self.toi_gian()
        b = other.toi_gian()
    ...
    def __str__(self) -> str:
        tg = self.toi_gian()
        if tg._mau == 1:
            return str(tg._tu)          # số nguyên nếu mẫu = 1
...
def demo():
    ...
    ps1 = PhanSo(1, 2)
    ps2 = PhanSo(1, 3)
    ps3 = PhanSo(2, 4)   # = 1/2 (trùng với ps1)
    ps4 = PhanSo(3, 1)   # = 3   (mẫu = 1)
    ps5 = PhanSo(-5, 6)
    ...
    print(f"\n  {'─'*50}")
    print("  So sánh:")
    print(f"    ps1 == ps3 : {ps1 == ps3}  (1/2 == 2/4)")
    print(f"    ps1 >  ps2 : {ps1 > ps2}   (1/2 > 1/3)")
    print(f"    ps5 <  ps2 : {ps5 < ps2}   (-5/6 < 1/3)")
...
def ung_dung():
    """Nhập dãy phân số, in tối giản, sắp xếp tăng dần."""
    print("\n" + "═"*55)
    ...
    print("\n  Sắp xếp tăng dần (dùng sorted() + __lt__):")
    for ps in sorted(ds):
        print(f"    {ps}")
